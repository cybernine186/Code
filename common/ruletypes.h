...
RULE_BOOL(Pets, LegacyRegen, false, "Enable legacy regen on pets by disabling OOC regen and ")
RULE_CATEGORY_END()
...
RULE_INT(Combat, NPCBashKickLevel, 6, "The level that NPC can KICK/BASH")
RULE_INT(Combat, NPCBashKickFrequency, 75, "The frequency that NPC can KICK/BASH, default 75. Warrior mobs both kick and bash, kick about 75% of the time")
...
RULE_BOOL(Combat, LegacyCombatFormulas, false, "Use legacy combat formula calculations over modern for a classic experience.")
RULE_BOOL(Combat, LOSCheckOnMelee, true, "Line of sight check on melee auto attack. Disable for increased performance.")
RULE_BOOL(Combat, LegacyMonkTigerClawTimer, false, "RoF2+ Tiger Claw is unlinked from other monk skills by default. false = Default (RoF2+), true = Legacy Timer (Classic).")
RULE_CATEGORY_END()
...
RULE_INT(NPC, OOCRegen, 0, "Enable out-of-combat regeneration for NPC, used as a perect to heal NPC's per tic in the following formula: (GetMaxHP() * ooc_regen / 100).")
...
RULE_BOOL(Cheat, MarkMQWarpLT, false, "Mark clients making smaller warps")
RULE_CATEGORY_END()

RULE_CATEGORY(PVP)
RULE_REAL(PVP, PVPHitBoxMod, 0.60, "PVP Hitbox Modifier size_mod * PVPHitBoxMod.")
...
RULE_INT(PVP, HitChanceBonusAttack, 0, "Hit chance bonus attack for normal melee, values range from 1-100.")
RULE_INT(PVP, HitChanceBonusSkillAttack, 0, "Hit chance skill attack for skill melee, values range from 1-100.")
RULE_INT(PVP, HitChanceBonusThrowingAttack, 0, "Hit chance bonus attack for throwing melee, values range from 1-100.")
RULE_INT(PVP, HitChanceBonusArcheryAttack, 0, "Hit chance bonus attack for archery melee, values range from 1-100.")
RULE_INT(PVP, HitChanceBonusSpecialAttack, 0, "Hit chance bonus attack for special melee, values range from 1-100. Skills: Backstab, Bash, Frenzy, Kick, and all Monk Special Attacks.")
RULE_INT(PVP, MeleeMitigationAC, 80, "Melee AC mitigated by this amount, default 80.")
RULE_INT(PVP, PetDamageMitigation, 100, "In PvP, pet damage is mitigated by this amount.")
RULE_BOOL(PVP, EnableGuardFactionAssist, true, "Enables faction based assisting against the aggresor in pvp.")
RULE_BOOL(PVP, AllowFear, false, "If enabled, will allow clients to fear eachother in PVP")
RULE_INT(PVP, Settings, 0, "Sets the PVP settings for the server. 1=Rallos Zek Ruleset, 2=Tallon/Vallon Zek (Size) Ruleset, 3=Tallon/Vallon Zek (Guild) Ruleset, 4=Sullon Zek Ruleset, 6=Discord Ruleset, anything above 6 is the Discord Ruleset without the no-drop restrictions removed. NOTE: edit IsAttackAllowed in Zone-table to accomodate for these rules.")
RULE_INT(PVP, MinLevel, 0, "Minimum level to pvp.")
RULE_INT(PVP, LevelDifference, 0, "In PvP, if value is greater than 0, players with a difference greater than value will not be attackable.")
RULE_INT(PVP, LoseExperienceLevelDifference, 0, "In PvP, if value is greater than 0, players lose experience if killed by a player within level difference.")
...
RULE_INT(PVP, BaseScore, 2, "PVP Point System Base Score.")
RULE_INT(PVP, LevelWeightMultiplier, 2, "PVP Point System Level Weight Multiplier.")
...
RULE_INT(PVP, PointsCap, 32, "PVP Point System Cap Per Player.")
RULE_INT(PVP, RespawnManaPercent, 100, "How much mana to respawn with.")
RULE_BOOL(PVP, IsAutoAttackAlwaysLoS, false, "When set to true, auto attack in pvp always has line of sight.")
RULE_BOOL(PVP, FacingInMeleeRequired, false, "When set to false, pvp melee can attack 360 degrees instead of just front.")
RULE_BOOL(PVP, CanLootCoin, false, "Can players loot coin from player corpses?")
RULE_BOOL(PVP, SummonedPetTreatedAsOwnerForResists, true, "")
RULE_INT(PVP, RootBreakFromSpellsByClient, 75, "Chance for root to break when cast on by a client.")
RULE_CATEGORY_END()
